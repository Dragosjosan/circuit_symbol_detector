...
def check_if_input_files_exist() -> None:
    ...
    if not all((DRAWINGS_TO_DETECT_IN.exists(), DRAWINGS_TO_DETECT_IN.is_dir())):
        logger.error(f"Drawings folder to detect in not found, or is not a directory: {DRAWINGS_TO_DETECT_IN}")
        raise FileNotFoundError(f"Drawings folder to detect in not found: {DRAWINGS_TO_DETECT_IN}")
    ...
    if not files:
        ...
        raise FileNotFoundError(f"No files found in drawings directory: {DRAWINGS_TO_DETECT_IN}")
    
    valid_files = [file for file in files if file.is_file() and file.suffix.lower() in ALLOWED_DRAWING_FILE_EXTENSIONS]
    if not valid_files:
        logger.error(f"No valid image files found in drawings directory: {DRAWINGS_TO_DETECT_IN}")
        raise FileNotFoundError(f"No valid image files found in drawings directory: {DRAWINGS_TO_DETECT_IN}")
...
def get_files_to_detect_in() -> List[Path]:
    return [
        file for file in DRAWINGS_TO_DETECT_IN.iterdir() 
        if file.is_file() and file.suffix.lower() in ALLOWED_DRAWING_FILE_EXTENSIONS
    ]

def process_files(files: List[Path], template: np.ndarray, width: int, height: int) -> None:
    for file in files:
            try:
                ...
                found_images = find_symbol_in_image_for_multiple_angles(image=main_image_gray, template=template)

                logger.info("Applying non-maximum suppression")
                final_matches = non_max_suppression(matches=found_images, width=width, height=height)

                logger.info("Drawing results")
                save_image_results(result_image=main_image, matches=final_matches, file_path=file, width=width, height=height)
...
def find_symbol_in_image_for_multiple_angles(image: np.ndarray, template: np.ndarray) -> List[Tuple[Tuple[int, int], float, float]]:
    all_matches = []
...
def save_image_results(result_image: np.ndarray, matches: List, file_path: Path, width: int, height: int) -> None:
    result_image = result_image.copy()
    for pt, angle, conf in matches:
        cv.rectangle(result_image, pt, (pt[0] + width, pt[1] + height), (0, 0, 255), 2)
        cv.putText(result_image, f"{angle}°, {conf:.2f}",
                    (pt[0], pt[1] - 5), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
def main() -> None:
    try:
        ...
        process_files(files=files_to_detect_in, template=template, width=width, height=height)
